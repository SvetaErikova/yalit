!(function (t, e) {
	...
	class E extends g {
		get isTouchDevice() {
			return null === S && (S = window.matchMedia("(hover: none)").matches), S;
		}
		get isMobile() {
			return (
				null === T && (T = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent)), T
			);
		}
		get panMode() {
			return this.options.panMode !== P || this.isTouchDevice ? x : P;
		}
		get panOnlyZoomed() {
		...
		constructor(t, e = {}, i = {}) {
			...
			(this.container = t),
				this.initContent(),
				this.attachPlugins(Object.assign(Object.assign({}, E.Plugins), i)),
				this.emit("init");
		...
		initContent() {
			const { container: t } = this,
				e = this.cn(O);
			let i = this.option(O) || t.querySelector(`.${e}`);
			if (
				(i ||
		...
		onLoad() {
			this.spinner && (this.spinner.remove(), (this.spinner = null)),
				...
				this.emit("afterLoad"),
				this.state === f.Init ? this.enable() : this.updateMetrics();
		}
		onError() {
		...
		attachObserver() {
			...
			const e = () =>
				...
				Math.abs(
					this.containerRect.height - this.container.getBoundingClientRect().height,
				) > 0.1;
			this.resizeObserver ||
		...
		attachEvents() {
			...
			t.addEventListener("click", this.onClick, { passive: !1, capture: !1 }),
				...
				document.addEventListener(P, this.onMouseMove);
		}
		detachEvents() {
			var t;
			...
			e.removeEventListener("click", this.onClick, { passive: !1, capture: !1 }),
				...
				document.removeEventListener(P, this.onMouseMove),
		...
		checkBounds(t = 0, e = 0) {
			...
			let d = 0,
				g = 0;
			return (
				h !== 1 / 0 && n < h ? (d = h - n) : c !== 1 / 0 && n > c && (d = c - n),
				l !== 1 / 0 && s < l ? (g = l - s) : u !== 1 / 0 && s > u && (g = u - s),
		...
		onClick(t) {
			...
			if (
				this.dragStart.time &&
				!this.canZoomOut() &&
				(Math.abs(h.x - this.dragStart.x) > 2 ||
					Math.abs(h.y - this.dragStart.y) > 2)
			)
				return;
			...
			const c = (e) => {
				...
				u = this.option("dblClick", t);
			u
				? (this.clicks++,
					1 == this.clicks &&
						(this.clickTimer = setTimeout(() => {
							1 === this.clicks
								? (this.emit("click", t), !t.defaultPrevented && l && c(l))
								: (this.emit("dblClick", t), t.defaultPrevented || c(u)),
								(this.clicks = 0),
		...
		onPointerDown(t, e, i) {
			...
			if (
				...
				return !1;
			if (this.panMode === P && this.targetScale > 1)
		...
		onPointerUp(t, e, n) {
			...
			this.container.classList.remove(this.cn("isDragging")),
				this.isDragging &&
					...
					(this.isDragging = !1),
					(this.lockedAxis = !1),
					this.state !== f.Destroy && (t.defaultPrevented || this.startDecelAnim()));
		}
		startDecelAnim() {
			var t;
			...
			Math.abs(v) < w && Math.abs(y) < w && ((l = u = 0), (d = g = 0)),
				(p < this.minScale - 1e-5 || p > this.maxScale + 1e-5 || (i && !l && !u)) &&
					(f = 0.35),
		...
		onWheel(t) {
			...
			if ((this.emit("wheel", t, i), this.panMode === P)) return;
		...
		updateMetrics(t = !1) {
			...
			const u = parseFloat(h.paddingTop) + parseFloat(h.paddingBottom),
				...
				g = l - u;
			this.containerRect = { width: c, height: l, innerWidth: d, innerHeight: g };
			...
			b.style.setProperty("transform", "none", "important"),
				r && ((b.style.width = ""), (b.style.height = "")),
				b.offsetHeight;
			...
			r &&
				...
				(b.style.cssText = v),
				r && ((b.style.width = `${w}px`), (b.style.height = `${M}px`)),
				this.setTransform(),
		...
		getBounds() {
			...
			if (!0 === h || (this.lockedAxis && h === this.lockedAxis))
				...
				let { containerRect: t, contentRect: h } = this,
					c = e(this.contentRect.fitWidth * this.targetScale, M),
					l = e(this.contentRect.fitHeight * this.targetScale, M),
					{ innerWidth: u, innerHeight: d } = t;
				if (
					(this.containerRect.width === c && (u = t.width),
					this.containerRect.width === l && (d = t.height),
					i > u)
				) {
					(r = 0.5 * (i - u)), (s = -1 * r);
				...
				if (
					(this.contentRect.fitWidth > u &&
						i < u &&
						((s -= 0.5 * (this.contentRect.fitWidth - u)),
						(r -= 0.5 * (this.contentRect.fitWidth - u))),
					n > d)
				) {
					(a = 0.5 * (n - d)), (o = -1 * a);
				...
				this.contentRect.fitHeight > d &&
					...
					(r -= 0.5 * (this.contentRect.fitHeight - d)));
			}
		...
		updateControls() {
			...
			let u =
					{
						toggleMax: a - l < 0.5 * (c - l) ? c : l,
						toggleCover: a - l < 0.5 * (h - l) ? h : l,
						toggleZoom: a - l < 0.5 * (r - l) ? r : l,
					}[t.option("click") || ""] || l,
				d = t.canZoomIn(),
				g = t.canZoomOut(),
				f = g && n === x;
			e(a) < e(l) && !this.panOnlyZoomed && (f = !0),
				(e(o.width, 1) > e(o.fitWidth, 1) || e(o.height, 1) > e(o.fitHeight, 1)) &&
					(f = !0),
				e(o.width * a, 1) < e(o.fitWidth, 1) && (f = !1),
				n === P && (f = !1);
			let m = d && e(u) > e(a),
				p = !m && !f && g && e(u) < e(a);
			s(i, this.cn("canZoomIn"), m),
				s(i, this.cn("canZoomOut"), p),
				s(i, this.cn("isDraggable"), f);
		...
		applyChange({
			...
			let v = new DOMMatrix().translate(g, p).translate(o, r).translate(t, i);
			if (this.option("zoom")) {
				if (!l) {
			...
			(v = v.translate(-o, -r).translate(-g, -p).multiply(b)),
				s && (v = v.rotate(s)),
				h && (v = v.scale(-1, 1)),
				c && (v = v.scale(1, -1));
			for (const t of m)
				"e" !== t &&
				"f" !== t &&
				(v[t] > this.minScale + 1e-5 || v[t] < this.minScale - 1e-5)
					? (this.target[t] = v[t])
					: (this.target[t] = e(v[t], M));
			(this.targetScale < this.scale ||
				Math.abs(n - 1) > 0.1 ||
				this.panMode === P ||
				!1 === u) &&
		...
		panWithMouse(t, i = this.option("mouseMoveFriction")) {
			if (((this.pmme = t), this.panMode !== P || !t)) return;
		...
		zoomTo(
			...
			this.stop();
			let r = 1;
			if ((this.panMode === P && (s = this.pmme || s), s)) {
			...
			"number" == typeof t
				...
				: ("next" === t &&
						(a - c < 0.2 && (c = a),
						(t = o < a - 1e-5 ? "full" : o < h - 1e-5 ? "max" : "fit")),
					(r =
						"full" === t
							? a / o || 1
							: "cover" === t
								...
								: "max" === t
									? h / o || 1
									: 1 / o || 1)),
				(e = "auto" === e ? (r > 1 ? 0.15 : 0.25) : e),
				this.applyChange({ scale: r, originX: i, originY: n, friction: e }),
				s && this.panMode === P && this.panWithMouse(s, e);
		}
		rotateCCW() {
		...
		reset(t) {
			if (this.state !== f.Init && this.state !== f.Destroy) {
				this.stop("current");
				for (const t of m) this.target[t] = y[t];
